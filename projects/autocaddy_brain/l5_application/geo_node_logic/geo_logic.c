...
static float calculate_distance_rc_car_to_destination_in_meters(void) {
  const int Earth_Radius_in_km = 6371;
  const float delta_lat = (current_RC_CAR_location.GPS_CURRENT_LAT - (float)phone_location.latitude) * (PI / 180);
  const float delta_long = (current_RC_CAR_location.GPS_CURRENT_LONG - (float)phone_location.longitutde) * (PI / 180);
  float a = pow(sinf(delta_lat / 2), 2) + cosf((float)phone_location.latitude * (PI / 180)) *
